...
# -------------------------
# débit (15-min à journalier moyen)
# -------------------------
...
val = scaler.transform(valid_set)
...
with torch.no_grad():
    ...
    # Titles and labels
    fig.suptitle('Loss (MSE) and NSE over training epochs', size=14)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Loss (MSE)")
    metric_ax.set_ylabel("Nash–Sutcliffe Efficiency (NSE)")
...
def permutation_importance(
    ...
    X : torch.Tensor (N, seq_len, n_features)
    y : torch.Tensor (N,)
    ...
    Returns
    -------
    dict with mean loss increase per feature
    """
    ...
    with torch.no_grad():
        ...
        baseline_loss = loss_func(baseline_preds, y).item()
    ...
    for i, fname in enumerate(feature_names):
        ...
        for _ in range(n_repeats):
            ...
            # Permute feature i along the sample dimension
            perm_idx = torch.randperm(X_perm.shape[0])
            X_perm[:, :, i] = X_perm[perm_idx, :, i]

            with torch.no_grad():
                preds = model(X_perm)
                loss = loss_func(preds, y).item()

            losses.append(loss)

        mean_loss = np.mean(losses)
        importances[fname] = mean_loss - baseline_loss

        print(
            f"{fname:15s} | "
            f"Δloss = {importances[fname]:.4f}"
        )
...
imp_df = pd.DataFrame.from_dict(
    ...
    orient="index",
    columns=["Δ loss"]
).sort_values("Δ loss", ascending=False)

plt.figure(figsize=(6, 4), dpi=150)
plt.barh(imp_df.index, imp_df["Δ loss"], color="steelblue")
plt.gca().invert_yaxis()
plt.xlabel("Increase in loss after permutation")
