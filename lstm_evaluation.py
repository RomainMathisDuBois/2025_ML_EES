...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 17:37:31 2025
...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 15 21:32:23 2025
...
############# on rend les données lisibles pour un LSTM ###################
...
################ evaluation du modele le plus performant #######################
...
with torch.no_grad():
    ...
    ax.set_ylabel('Discharge ($\\frac{m3}{s}$)', size=11)
    ax.autoscale(enable=True, axis='x', tight=True)

    with torch.no_grad():
          sim_full = model(test_Xtensor).cpu()
          obs_full = test_ytensor.cpu()
          KGE = calc_kge(sim_full, obs_full)
      
      
    
    fig.suptitle('Performance on Test Dataset', size=14)
    fig.set_facecolor('lightgrey')
    ax.set_title(f'MSE: {mse:0.2f}  NSE: {NSE:0.2f}  KGE: {KGE:0.2f}', size=11)


#################### evaluation sur les 3 set ####################################
...
checkpoint = torch.load("best_model.pt", map_location=calc_device)

# Cas 1 : tu as sauvegardé uniquement state_dict
model.load_state_dict(checkpoint)

# Cas 2 : tu as sauvegardé un dict (ce que tu fais maintenant)
# model.load_state_dict(checkpoint["model_state_dict"])

model.eval()

def calc_nse_full(sim, obs):
    obs_mean = obs.mean()
    numerator = torch.sum((sim - obs) ** 2)
    denominator = torch.sum((obs - obs_mean) ** 2)
    return 1 - numerator / denominator

def calc_kge_full(sim, obs):
    r = torch.corrcoef(torch.stack([sim, obs]))[0, 1]
    alpha = sim.std() / obs.std()
    beta = sim.mean() / obs.mean()
    return 1 - torch.sqrt((r - 1)**2 + (alpha - 1)**2 + (beta - 1)**2)

def evaluate_dataset(model, dataloader, loss_func):
    model.eval()
    ...
    with torch.no_grad():
        for X_batch, y_batch in dataloader:
            ...
            obs_all.append(y_batch.cpu())

    sim = torch.cat(preds_all)
    obs = torch.cat(obs_all)

    loss = loss_func(sim, obs).item()
    NSE = calc_nse_full(sim, obs).item()
    KGE = calc_kge_full(sim, obs).item()

    return loss, NSE, KGE

train_loss, train_NSE, train_KGE = evaluate_dataset(
    model, train_loader, loss_func
)

val_loss, val_NSE, val_KGE = evaluate_dataset(
    model, val_loader, loss_func
)

test_loss, test_NSE, test_KGE = evaluate_dataset(
    model, test_loader, loss_func
)

print("===== MODEL PERFORMANCE =====")
print(f"TRAIN | Loss: {train_loss:.2f} | NSE: {train_NSE:.3f} | KGE: {train_KGE:.3f}")
print(f"VALID | Loss: {val_loss:.2f} | NSE: {val_NSE:.3f} | KGE: {val_KGE:.3f}")
print(f"TEST  | Loss: {test_loss:.2f} | NSE: {test_NSE:.3f} | KGE: {test_KGE:.3f}")
